...
// Async check key
keyCheck.checkKey().then(function (keyResult) {
  if (!keyResult.valid) {
    console.log(chalk.red("═══════════════════════════════════════"));
    console.log(chalk.red("⛔ KEY KHÔNG HỢP LỆ HOẶC ĐÃ HẾT HẠN!"));
    console.log(chalk.red("═══════════════════════════════════════"));
    console.log(chalk.yellow(keyResult.message));
    console.log(chalk.red("═══════════════════════════════════════"));
    process.exit(1);
  }
  if (keyResult.message && !keyResult.isAdmin) {
    console.log(chalk.green("═══════════════════════════════════════"));
    console.log(chalk.green(keyResult.message));
    console.log(chalk.green("═══════════════════════════════════════"));
  }

  // Bắt đầu auto-check mỗi 10 phút
  keyCheck.startAutoCheck();
});
// ========================================================
...
fs.readFile('package.json', 'utf8', (err, data) => {
  ...
  // Đã bỏ check lỗi modules ở đây vì main.js sẽ làm việc này
  // Điều này giúp tăng tốc khởi động đáng kể
});
...
async function startb() {
  ...
  if (config.ACCESSTOKEN !== "") {
    ...
    setTimeout(function () {
      startBot();
    }, 3000); // Giảm từ 7000ms xuống 3000ms để tăng tốc khởi động
  }
}
startb();
